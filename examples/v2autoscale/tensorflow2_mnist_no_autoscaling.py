...
def train(state):
    for state.epoch in range(state.epoch, args.num_epochs):
        ...
        for batch_idx, (images, labels) in enumerate(dataset.skip(state.batch).take(batches_train)):
            ...
            # print("Local throughput average: {}".format(local_throughput_average))
            # global_throughput = comm.reduce(local_throughput_average, op=MPI.SUM, root=0)
            global_throughput = local_throughput_average * hvd.size()
            if hvd.rank() == 0:
                print("Batch {}\t| global throughput: {}".format(state.batch, global_throughput))
...
state = hvd.elastic.TensorFlowKerasState(mnist_model, opt, epoch=0, batch=0, start_time=time.time())
